...
│   └── archive/           # Older meeting notes (auto-archived after 30 days)
...
- **Meeting notes are auto-archived.** Notes older than 30 days are moved to `meetings/archive/` when you run `/end`. Key data (decisions, action items) is always preserved in their dedicated stores. Wikilinks still work in Obsidian regardless of subfolder.
